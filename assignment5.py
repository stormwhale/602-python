...
def exercise01():
    ...
    df = pd.DataFrame(
        {"a": [1, 2, 3], "b": [4, 5, 6], "c": [8, 6, 6], "d": [454, 343, 2123]}
    )
...
def exercise13(n):
    ...
    date = pd.date_range(start="1/1/2010", periods=n, freq="D")

    # create the random number
    rando = np.random.randint(0, n, size=n)
    s = pd.Series(rando, index=date)

    # plots:
    plt.plot(
        s.index, s.cumsum()
    )  # This is to add on the number from the previous day to the current.
    plt.xticks(rotation=45)
    plt.title("Date with cumculative numbers")
    plt.show()
    # ------ Place code above here /\ /\ /\ ------
    return s
...
def exercise14(words):
    ...
    length = series.map(lambda x: len(x))
    # combine the two into one dataframe
    df = pd.DataFrame({"words": series, "word length": length})
    # ------ Place code above here /\ /\ /\ ------
    return df
...
def exercise15():
    ...
    """
    # ------ Place code below here \/ \/ \/ ------
    df = pd.read_csv("https://tinyurl.com/y63q7okz").iloc[
        lambda x: x.index % 5 == 0, [0, 2]
    ]
    # ------ Place code above here /\ /\ /\ ------
    return df
